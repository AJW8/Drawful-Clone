...
io.on('connection', function(socket){
	try{
		...
		socket.on('game_submit_drawing', function(drawing){
			if(isPlayer(socket, session)){
				var game = games.getGame(session.game_id);
				if(game) game.receiveDrawing(session.user_id, drawing);
			}
		});
		socket.on('game_submit_title', function(title){
			if(isPlayer(socket, session)){
				var game = games.getGame(session.game_id);
				if(game) game.receiveTitle(session.user_id, title);
			}
		});
